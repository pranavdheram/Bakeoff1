...
void setup()
{
  ...
  String path = "G:/CarnegieMellon/Spring-1/DHCS/bakeoff1/FinalVersion";
  //path = path.replace("\\", "/");
...
void mousePressed() // test to see if hit was in target!
{
 ...
 //check to see if mouse cursor is inside button 
  if ((mouseX > bounds.x-25  && mouseX < bounds.x + bounds.width + 25) && (mouseY > bounds.y - 25 && mouseY < bounds.y + bounds.height + 25)) // test to see if hit was within bounds
  {
    ...
    hits++;
    try {
      bw.write(trialNum + "," + participantId + "," + currMouseX + "," + currMouseY + "," + (bounds.x+(bounds.width/2)) + "," + (bounds.y+(bounds.height/2)) + "," + bounds.width + "," + (millis()-currTime)/1000.000 + "," + 1);
      bw.newLine();
    } catch(Exception e) {
      e.printStackTrace();
    }
  ...
  else
  {
    ...
    misses++;
    try {
      bw.write(trialNum + "," + participantId + "," + currMouseX + "," + currMouseY + "," + (bounds.x+(bounds.width/2)) + "," + (bounds.y+(bounds.height/2)) + "," + bounds.width + "," + (millis()-currTime)/1000.000 + "," + 0);
      bw.newLine();
    } catch(Exception e) {
      e.printStackTrace();
    }
